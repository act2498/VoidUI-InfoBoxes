...
elseif RequiredScript == "lib/managers/hud/hudobjectives" then
    if  VoidUI.options.enable_objectives then
        ...
        function HUDObjectives:sort_boxes()
            ...
            for priority, panels in pairs(managers.hud._hud_objectives._custom_icons) do
                for i,panel in ipairs(managers.hud._hud_objectives._custom_icons[priority]) do
                    if alive(panel) then
                        PrintTable(panel)
                    end
                end
    ...
    function HUDObjectives:change_buff_timer(data)
        ...
        if data.id and data.time and self._timer[data.id] then
            ...
            elseif data.operation and data.operation == "reset" or data.operation == "set_time" then
                VoidUIInfobox:child(data.id)._init_time = data.time
                self._timer[data.id] = data.time
            end
        end
        if force_remove_when_finished then
            managers.hud:remove_updator("buff_"..data.id)
            managers.hud:add_updator("buff_"..data.id, callback(self, self, "update_buff_timer", data))
        end
    end
